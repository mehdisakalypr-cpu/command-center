...
export async function scoreSupportCase(ticket: { subject: string; body: string; expected_disposition: string }, actualResponse: string) {
  ...
  const gen = await withFallback(
    { system: SUPPORT_RUBRIC_SYSTEM + STRICT_JSON_DIRECTIVE, prompt, model: 'llama-4-scout-17b-16e-instruct', temperature: 0, maxTokens: 600 },
    { project: 'cc', order: ['gemini','mistral','groq','openrouter'] },
  );
  return extractJSON<{ disposition_match: 0|1; resolved_without_human: 0|1 }>(stripPreamble(gen.text));
}
