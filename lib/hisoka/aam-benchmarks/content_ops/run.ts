...
export async function scoreContentCase(post: string, expectedWordMin: number): Promise<{ overall: number; breakdown: unknown }> {
  ...
  const gen = await withFallback(
    { system: CONTENT_OPS_RUBRIC_SYSTEM + STRICT_JSON_DIRECTIVE, prompt, model: 'llama-4-scout-17b-16e-instruct', temperature: 0, maxTokens: 1000 },
    { project: 'cc', order: ['gemini','mistral','groq','openrouter'] },
  );
  const parsed = extractJSON<{ originality: number; factuality: number; seo_signals: number; length_voice: number; overall: number }>(stripPreamble(gen.text));
