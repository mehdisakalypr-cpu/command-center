...
export async function scoreFulfillmentCase(
  ...
  const gen = await withFallback(
    { system: FULFILLMENT_RUBRIC_SYSTEM + STRICT_JSON_DIRECTIVE, prompt, model: 'llama-4-scout-17b-16e-instruct', temperature: 0, maxTokens: 500 },
    { project: 'cc', order: ['gemini','mistral','groq','openrouter'] },
  );
  return extractJSON<{ artifact_match: 0|1; within_sla: 0|1 }>(stripPreamble(gen.text));
}
