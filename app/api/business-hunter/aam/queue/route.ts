...
export async function GET() {
  ...
  const [q, active, recent, budget] = await Promise.all([
    admin.from('business_ideas').select('id,name,autonomy_score,forge_attempts,forge_status,automation_gaps').eq('forge_status','idle').gte('autonomy_score',0.75).lte('autonomy_score',0.91).lt('forge_attempts',3).order('autonomy_score',{ascending:true}).limit(20),
