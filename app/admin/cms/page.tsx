...
const S = {
  ...
  tab: (active: boolean) => ({ padding: "8px 16px", fontSize: ".72rem", fontWeight: active ? 600 : 400, color: active ? "#C9A84C" : "#9BA8B8", background: active ? "rgba(201,168,76,.08)" : "transparent", border: active ? "1px solid rgba(201,168,76,.2)" : "1px solid transparent", borderBottom: "none", cursor: "pointer", fontFamily: "inherit", display: "flex", alignItems: "center", gap: 6 } as React.CSSProperties),
  collBtn: (active: boolean) => ({ display: "block", width: "100%", textAlign: "left" as const, padding: "8px 12px", fontSize: ".72rem", color: active ? "#E8E0D0" : "#9BA8B8", background: active ? "rgba(255,255,255,.06)" : "transparent", border: "none", cursor: "pointer", fontFamily: "inherit" } as React.CSSProperties),
  card: (dirty: boolean) => ({ background: "#0A1A2E", border: `1px solid ${dirty ? "rgba(201,168,76,.4)" : "rgba(255,255,255,.06)"}`, padding: "16px 18px", marginBottom: 10 } as React.CSSProperties),
  input: { width: "100%", padding: "8px 10px", background: "#071425", border: "1px solid rgba(255,255,255,.08)", color: "#E8E0D0", fontSize: ".72rem", fontFamily: "inherit", outline: "none" } as React.CSSProperties,
  textarea: { width: "100%", padding: "8px 10px", background: "#071425", border: "1px solid rgba(255,255,255,.08)", color: "#E8E0D0", fontSize: ".72rem", fontFamily: "monospace", outline: "none", resize: "vertical" as const, minHeight: 60 } as React.CSSProperties,
  ...
  smallBtn: { padding: "3px 8px", background: "rgba(255,255,255,.04)", border: "1px solid rgba(255,255,255,.06)", color: "#9BA8B8", fontSize: ".58rem", cursor: "pointer", fontFamily: "inherit" } as React.CSSProperties,
  ...
  modal: { position: "fixed" as const, inset: 0, zIndex: 200, background: "rgba(0,0,0,.6)", display: "flex", alignItems: "center", justifyContent: "center", padding: 20 },
  modalInner: { background: "#0A1A2E", border: "1px solid rgba(201,168,76,.2)", padding: "24px", width: "100%", maxWidth: 500, maxHeight: "70vh", overflow: "auto" } as React.CSSProperties,
};
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", height: "100%", color: "#E8E0D0", fontFamily: "Inter, sans-serif" }}>
      ...
      <div style={S.header}>
        ...
        {hasDirty && (
          <button onClick={saveAll} disabled={saving} style={{ ...S.goldBtn, marginLeft: "auto" }}>
      ...
      {/* Site tabs */}
      <div style={{ display: "flex", gap: 0, borderBottom: "1px solid rgba(255,255,255,.06)", paddingLeft: 16 }}>
        {CMS_SITES.map(site => (
          <button key={site.key} onClick={() => { setActiveSite(site); setActiveColl(site.collections[0]); setDrafts({}); setHistSlug(null); }} style={S.tab(activeSite.key === site.key)}>
        ...
        <div style={S.modal} onClick={() => setHistSlug(null)}>
          <div style={S.modalInner} onClick={e => e.stopPropagation()}>
            <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 16 }}>
              ...
              <button onClick={() => setHistSlug(null)} style={{ background: "none", border: "none", color: "#5A6A7A", cursor: "pointer", fontSize: "1.2rem" }}>&times;</button>
            </div>
            {history.length === 0 ? (
                ...
                {history.map(h => (
                  <div key={h.id} style={{ background: "rgba(255,255,255,.03)", border: "1px solid rgba(255,255,255,.06)", padding: "10px 12px" }}>
                    <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 6 }}>
                      ...
                      <button onClick={() => rollback(h.id)} disabled={saving} style={{ ...S.goldBtn, padding: "3px 10px", fontSize: ".56rem" }}>Restaurer</button>
                    </div>
