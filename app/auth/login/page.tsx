...
export default function LoginPage() {
  ...
  useEffect(() => {
    ...
    const saved = localStorage.getItem('cc_biometric_email')
    if (saved) {
  ...
  async function resetBiometric() {
    localStorage.removeItem('cc_biometric_email')
    localStorage.removeItem('cc_biometric_offered')
    setBiometricAvailable(false)
    setBiometricEmail('')
    setError(null)
    try {
  ...
  async function handleLogin(e: React.FormEvent) {
    ...
    if (effectiveRemember) {
      document.cookie = `cc_remember=1; path=/; max-age=${60 * 60 * 24 * 90}; SameSite=Lax; Secure`
    } else {
      const v = localStorage.getItem(STORAGE_KEY)
      if (v) { sessionStorage.setItem(STORAGE_KEY, v); localStorage.removeItem(STORAGE_KEY) }
      document.cookie = 'cc_remember=; path=/; max-age=0; SameSite=Lax; Secure'
    }
    ...
    setLoading(false)
    router.push(new URLSearchParams(window.location.search).get('next') || '/admin/overview')
  }
  ...
  return (
    <div className="min-h-screen bg-[#07090F] flex items-center justify-center px-4">
      <div className="w-full max-w-sm">
        {/* Logo */}
        <Link href="/admin/overview" className="flex items-center justify-center gap-2 mb-8">
          ...
          <span className="font-bold text-white">Command <span className="text-[#C9A84C]">Center</span></span>
        </Link>
